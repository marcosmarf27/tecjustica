...
2. Avise o usuário: "A sessão do PJE não está ativa nessa aba. Faça login manualmente em `https://pje.tjce.jus.br/` e me avise quando puder."
...
O CNJ não existe no PJE-CE 1º grau público. Causas prováveis, em ordem de frequência:

1. **Segredo de justiça sem permissão** — você consegue ver o processo se estiver como parte/advogado cadastrado, mas a consulta pública nega. Não há como contornar via Consulta Processual; seria necessário o caminho via Tarefas (não implementado por escolha de design).
2. **Processo do 2º grau** — se o CNJ tem `.8.06.` mas o processo está em fase recursal, pode estar em `pje2grau`. A skill não suporta.
3. **CNJ digitado errado** — especialmente o dígito verificador. Peça ao usuário pra conferir.
4. **Processo arquivado/migrado para sistema legacy** (raro).

**Faça:** reporte "Processo X não encontrado no PJE-CE 1º grau público" e siga com os próximos CNJs da lista. Não tente fallback automático.

## Sintoma: "Aba MinIO não apareceu em 3 minutos"
...
A URL assinada do MinIO expirou. Você demorou mais de 120s entre capturar a URL e chamar curl. Alternativa: a URL foi copiada errado (faltando um query param).
...
3. Se ficar alternando 403/200, é sinal de que você está demorando no meio do fluxo. Minimize passos entre a detecção da aba MinIO e o `curl`: capture a URL e baixe imediatamente, só faça logging/verificação depois.

## Sintoma: "curl retornou HTTP 200 mas o arquivo tem 0 bytes"

Muito raro. Indica que o MinIO aceitou a requisição mas o objeto tem tamanho zero — bug no servidor.
...
O `curl` pegou um HTML de erro do MinIO, não o PDF.
...
## Sintoma: "Duas abas PJE-CE abertas, sessões conflitam"
...
**Faça:**
- Para URL do MinIO: pegue do retorno do `tabs_context_mcp` (campo `url`). Não tente contornar com `location.search` ou `atob` — o bloqueio é intencional.
